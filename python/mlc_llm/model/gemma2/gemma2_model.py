...
class Gemma2Config(ConfigBase):  # pylint: disable=too-many-instance-attributes
    """Configuration of the Gemma2 model."""

    attention_bias: bool
    #TODO not implemented, but according to https://huggingface.co/blog/gemma2 The Gemma 2 team observed very minor differences when soft-capping is removed during inference.
    attn_logit_softcapping: float
    #TODO this is applied at the sampling stage
...
class Gemma2Embedding(nn.Embedding):
    """The embedding module specialized for Gemma2 so that
...
class Gemma2Attention(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: Gemma2Config):
        self.head_dim = config.head_dim
        self.scaling_factor = config.query_pre_attn_scalar**-0.5
    ...
    def forward(self, hidden_states: Tensor, paged_kv_cache: PagedKVCache, layer_id: int):
        ...
        # Attention
        output = op.reshape(
            paged_kv_cache.attention_with_fused_qkv(layer_id, qkv, self.num_q_heads, self.scaling_factor),
            (b, s, h_q * d),
        )
...
class Gemma2DecoderLayer(nn.Module):
    def __init__(self, config: Gemma2Config):
        ...
        self.pre_feedforward_layernorm = nn.RMSNorm(config.hidden_size, -1, rms_norm_eps, bias=False)
        self.post_feedforward_layernorm = nn.RMSNorm(config.hidden_size, -1, rms_norm_eps, bias=False)
...
class Gemma2Model(nn.Module):
    def __init__(self, config: Gemma2Config):
        ...
        self.embed_tokens = Gemma2Embedding("vocab_size", config.hidden_size)
...
class Gemma2ForCausalLM(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: Gemma2Config):
