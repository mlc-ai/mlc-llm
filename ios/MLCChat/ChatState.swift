...
class ChatState : ObservableObject {
    ...
    func interruptable() -> Bool {
        return getModelChatState() == .Ready || getModelChatState() == .Generating || getModelChatState() == .Failed || getModelChatState() == .PendingImageUpload || getModelChatState() == .ProcessingImage
    }
    ...
    private func interruptChat(prologue: () -> Void, epilogue: @escaping () -> Void) {
        ...
        if getModelChatState() == .Ready || getModelChatState() == .Failed || getModelChatState() == .PendingImageUpload {
            prologue()
            epilogue()
        } else if getModelChatState() == .Generating || getModelChatState() == .ProcessingImage {
            prologue()
            threadWorker.push {
    ...
    func requestProcessImage(image: UIImage) {
        ...
        threadWorker.push {[self] in
            ...
            // step 1. transform image and convert to bitmap
            let imageBitMap = transformImage(image: image, width: 112, height: 112)
            // step 2. prefill image by backend.prefillImage()
            backend.prefillImage(imageBitMap, prevPlaceholder: "<Img>", postPlaceholder: "</Img> ")
            DispatchQueue.main.async { [self] in
                updateMessage(role: .bot, message: "[System] Ready to chat")
                switchToReady()
            }
