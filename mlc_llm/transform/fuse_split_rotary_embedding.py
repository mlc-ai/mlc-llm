...
def fuse_split_rotary_embedding(
    ...
    def ir_module_pass(mod: tvm.IRModule, _pass_context) -> tvm.IRModule:
        ...
        with PatternContext() as ctx:
            ...
            pat_rotary_embedding_gvar = GlobalVarPattern("rotary_embedding") | GlobalVarPattern(
                "rotary_embedding1"
            )
            ...
            pat_embedded_query = is_op("relax.call_tir")(
                pat_rotary_embedding_gvar, TuplePattern([pat_query]), offset, add_constraint=False
            )
            pat_embedded_key = is_op("relax.call_tir")(
                pat_rotary_embedding_gvar, TuplePattern([pat_key]), offset, add_constraint=False
            )
        ...
        def rewriter(matchings, bindings):
            ...
            # Reproduce intermediates
            #
            # These will most likely be removed by DCE, as they were
            # previously intermediate values.  However, just in case
            # something else was using them, we should define them.
            flat_query_new = relax.op.reshape(
                qkv_tuple_new[0], [batch_size, seq_len, num_query_heads * head_dim]
            )
            flat_key_new = relax.op.reshape(
                qkv_tuple_new[1], [batch_size, seq_len, num_kv_heads * head_dim]
            )
            flat_value_new = relax.op.reshape(
                qkv_tuple_new[2], [batch_size, seq_len, num_kv_heads * head_dim]
            )
            flat_qkv_tuple_new = relax.Tuple([flat_query_new, flat_key_new, flat_value_new])

            return {
                flat_qkv_tuple: flat_qkv_tuple_new,
                flat_query: flat_query_new,
                flat_key: flat_key_new,
                flat_value: flat_value_new,
                value: value_new,
        ...
        assert "split_rotary" in str(new_mod["prefill"])
        return new_mod
