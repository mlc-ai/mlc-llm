...
class LLMImage {
 ...
 public:
   ...
   */
  NDArray EmbedStep(DLTensor* image) {
    CHECK(embed_func_.defined());
    auto tstart = std::chrono::high_resolution_clock::now();

    std::vector<ShapeTuple::index_type> shape;
    shape.resize(image->ndim);
    shape.assign(image->shape, image->shape + image->ndim);
    size_t nbytes = size_t(image->dtype.bits / 8);
    for (auto s : shape) {
      nbytes *= (size_t)s;
    }
    NDArray input_image = NDArray::Empty(ShapeTuple(shape), image->dtype, device_);
    input_image.CopyFromBytes(image->data, nbytes);
    NDArray embedding = embed_func_(input_image, params_);
