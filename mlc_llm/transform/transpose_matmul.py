        ...
        def te_transposed_matmul(a: te.Tensor, b: te.Tensor) -> te.Tensor:
            ...
            def matmul_compute(*idx_spatial):
                ...
                def multiply_compute(idx_reduce):
                    ...
                    return (a(*a_indices) * b(*b_indices)).astype(out_dtype)

                te_sum = tir.comm_reducer(
                    lambda x, y: x.astype(out_dtype) + y.astype(out_dtype),
                    lambda t: tir.const(0, dtype=out_dtype),
                    name="sum",
                )
                return te_sum(multiply_compute(k), axis=k)
