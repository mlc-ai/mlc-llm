...
from mlc_llm.dlight import sch_norm as S
from mlc_llm.dlight.mod_norm import Module
...
def main():
    ...
    for i in [1, 2, 3, 4, 5, 6]:
        ...
        try:
            if 'with T.block("Ared_temp"):' in func.script():
                name_norm = "Ared_temp"
            elif 'with T.block("A_red_temp"):' in func.script():
                name_norm = "A_red_temp"
            else:
                print(f"Weird workload. Skipping func{i}")
            sch = tir.Schedule(func)
            S.sch_norm(name_norm)(sch)
            build_and_measure(
                i,
                func,
                sch,
