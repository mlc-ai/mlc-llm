...
We expose Python API for compiling/building model in the package ``mlc_llm``, so
...
Afterwards, we use ``pip`` to install ``mlc_llm`` as a package so that we can
...
``mlc_llm`` even if you are not in the directory of ``mlc-llm``:
