...
MessagePlaceholders message_placeholder_from_string(const std::string& role) {
    static const std::unordered_map<std::string, MessagePlaceholders> enumMap = {
        {"system", MessagePlaceholders::SYSTEM},
        {"user", MessagePlaceholders::USER},
        {"assistant", MessagePlaceholders::ASSISTANT},
        {"tool", MessagePlaceholders::TOOL},
...
std::optional<std::vector<Data>> Conversation::as_prompt(std::string* err) {
    ...
    for (int i = 0; i < messages.size(); i++) {
        ...
        for (auto& item : content.value()) {
            ...
            if (item["type"] == "text") {
                ...
                std::string placeholder = PLACEHOLDERS[message_placeholder_from_string(role)];
...
std::optional<Conversation> Conversation::FromJSON(const picojson::object& json, std::string* err) {
    ...
    if (json::ParseJSONField(json, "system_prefix_token_ids", system_prefix_token_ids_arr, err, false)) {
    ...
    if (!json::ParseJSONField(json, "add_role_after_system_message", add_role_after_system_message, err, true)) {
        return std::nullopt;
    }
