...
class OlmoConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    ...
    def __post_init__(self):
        ...
        if self.prefill_chunk_size == 0:
            ...
            self.prefill_chunk_size = min(self.context_window_size, 8192)
        assert self.tensor_parallel_shards == 1, "Olmo currently does not support sharding."
...
class OlmoAttention(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: OlmoConfig):
        ...
        self.qkv_clip = config.clip_qkv
        self.qkv_proj = nn.Linear(
            in_features=config.hidden_size,
            out_features=(2 * self.num_key_value_heads + self.num_heads) * self.head_dim,
    ...
    def forward(self, hidden_states: Tensor, paged_kv_cache: PagedKVCache, layer_id: int):
        ...
        #     qkv_clamped = op.where(qkv_clamped > qkv_clip, qkv_clip, qkv_clamped)
        #     qkv = qkv_clamped
...
class OlmoForCausalLM(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: OlmoConfig):
